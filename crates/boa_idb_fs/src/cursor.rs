//! Lazy filesystem cursor: merge-iterates snapshot maps with the
...
//! into a `Vec` at `scan` time: a 200k walk peaked at ~165 MB. This cursor
//! holds structural (`O(1)`) clones of the committed maps taken at `scan`
//! plus the borrowed pending overlay, and re-seeks per step, so memory
//! stays O(1) in the range size. No locks are held across steps. Snapshot
//! isolation is preserved: the snapshot version is frozen at `scan` time
//! (persistent maps), unlike live re-reads.
...
use crate::state::{IndexKey, PersistMap, RecordKey};
...
/// Lazy merge cursor over snapshot maps plus the pending overlay.
pub struct FsCursor<'a> {
    /// Committed records frozen at `scan` time (structural clone).
    committed_records: PersistMap<RecordKey, Vec<u8>>,
    /// Committed index entries frozen at `scan` time.
    committed_index: PersistMap<IndexKey, ()>,
...
/// Snapshot + overlay maps a lazy cursor merges (M7-B H-MEM).
///
/// Bundles the four map arguments so cursor constructors stay under the
/// pedantic argument-count lint.
pub(crate) struct CursorMaps<'a> {
    pub(crate) records: PersistMap<RecordKey, Vec<u8>>,
    pub(crate) index: PersistMap<IndexKey, ()>,
    pub(crate) pending_records: &'a BTreeMap<RecordKey, Option<Vec<u8>>>,
    pub(crate) pending_index: &'a BTreeMap<IndexKey, bool>,
}

impl<'a> FsCursor<'a> {
    /// Opens a cursor of the given kind; fetches nothing until positioned.
    fn open(
        maps: CursorMaps<'a>,
        kind: CursorKind,
        range: EncodedRange,
    ...
    /// seek floor narrowed with the range lower bound (narrower of the two),
    ...
    /// Next snapshot store key at or after the resume point. Pure peek.
    fn peek_snapshot_record(&self, store: StoreId) -> Option<RecordKey> {
    ...
    /// Next pending store slot at or after the resume point. Pure peek.
    ...
    fn advance_store(&mut self, store: StoreId) -> Result<bool, BackendError> {
        ...
        loop {
            let committed = self.peek_snapshot_record(store);
            let pending = self.peek_pending_record(store);
            // (key, pending value if the winner is live, source, tombstoned?)
            let pick: Option<(RecordKey, Option<Vec<u8>>, Source, bool)> =
                match (committed, pending) {
                    (None, None) => None,
                    (Some(ckey), None) => {
                        let tombstoned =
                            self.pending_records.get(&ckey).is_some_and(Option::is_none);
                    ...
                    (Some(ckey), Some((pkey, slot))) => {
                        ...
                        match pkey.1.cmp(&ckey.1) {
                            ...
                            Ordering::Equal => {
                                // Same key: pending shadows the snapshot. Both
            ...
            if !self.range.contains(&key.1) {
                ...
                // skips with positions already advanced; above-upper or
                // cross-store ends the walk: ordering guarantees nothing
                // later matches.
    ...
    /// Next snapshot index key at or after the resume point. Pure peek.
    fn peek_snapshot_index(&self, index: IndexId) -> Option<IndexKey> {
        let found = if self.desc {
    ...
    /// Next pending index slot at or after the resume point. Pure peek.
    ...
    /// moves at least one source past the examined key.
    fn advance_index(
        &mut self,
        store: StoreId,
        index: IndexId,
        ...
        loop {
            let committed = self.peek_snapshot_index(index);
            ...
            let pick: Option<(Vec<u8>, Vec<u8>, Source, bool)> = match (committed, pending) {
                (None, None) => None,
                (Some(ckey), None) => {
                    let tombstoned = matches!(self.pending_index.get(&ckey), Some(false));
                ...
                (Some(ckey), Some((pkey, live))) => {
                        ...
                        .cmp(&(ckey.1.as_slice(), ckey.2.as_slice()))
                    {
                        ...
                        Ordering::Equal => {
                            // Same entry: pending shadows the snapshot. Both
                            // sources move past it exactly once.
            ...
            if unique {
                ...
                if self.desc {
                    return self.yield_unique_desc_group(store, index, &idx_key);
                }
    ...
    /// Buffers the representative row of a unique-index group while walking
    /// backwards: the smallest primary key of the group, resolved by a
    /// forward sub-seek. Jumps both descents below the whole group so larger
    /// members never resurface.
    fn yield_unique_desc_group(
        &mut self,
        store: StoreId,
        index: IndexId,
        idx_key: &[u8],
        ...
        self.last_index_key = Some(idx_key.to_vec());
        self.committed_index_pos = Some((index, idx_key.to_vec(), min_pkey.clone()));
        self.pending_index_pos = Some((index, idx_key.to_vec(), min_pkey.clone()));
        let value = if self.key_only { None } else { value };
        self.current = Some((idx_key.to_vec(), min_pkey, value));
        Ok(true)
    }
    ...
    fn group_min(
        ...
        // Snapshot member: smallest pkey at/after (idx, ⊥), tombstone-free.
        let mut snapshot_min: Option<(Vec<u8>, Option<Vec<u8>>)> = None;
        {
            let start: Bound<IndexKey> = Bound::Included((index, idx_key.to_vec(), bottom.clone()));
            for (key, _) in self.committed_index.range((start, Bound::Unbounded)) {
                let (iid, ik, pk) = key;
                ...
                snapshot_min = Some((pk.clone(), value));
                break;
            }
        }
        ...
        match (snapshot_min, pending_min) {
...
impl BackendCursor for FsCursor<'_> {
        ...
        match target {
            ...
            CursorSeek::Key(key) => match self.kind {
                ...
                CursorKind::Index { index, .. } => {
                    // Index seeks floor on (index key, smallest pkey).
