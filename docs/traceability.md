...
| R13.1 | Test levels: perf, memory, fuzz, crash | `criterion` + `scan-1m` (perf), `memory_gates_tests` + massif (memory), 5 libFuzzer targets × 48 min nightly (fuzz), kill-worker 200 nightly (crash) | PASS (all levels wired; first 4 h fuzz evidence pending inaugural nightly run) |
| R13.2 | Coverage thresholds (`boa_idb_core` ≥90 %, `boa_idb` ≥80 %, `llvm-cov` gate) | Nightly `llvm-cov` (tests + 3 WPT runs) with LCOV artifact + absolute threshold gate (core 90 / boa_idb 80, schema bug fixed); measured actuals core 90.37 / boa_idb 80.52 | PASS (absolutes enforced by the gate; engine modules + API edges covered by unit/scenario tests; no broad ignores used; uncoverable const-eval compensated and documented; dead-engine removal left as optional follow-up (breaking API change)) |
...
* R12–R13 (M7-B): same ids, statuses updated above (R12.1 PARTIAL with fixes, R12.2/R13.1/R13.6.1/R13.6.2 PASS, R13.2 PARTIAL with ratchet, fuzz PARTIAL pending inaugural nightly).
