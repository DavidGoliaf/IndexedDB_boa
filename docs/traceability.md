...
| R12.1 | Benchmark harness, 7 §12.1 scenarios (SQLite/FS + memory control), receipts, CI smoke, M7-B fixes | `crates/boa_idb/benches/backends.rs` (scan ids carry `N`, diagnostic), `examples/scan-1m.rs` (normative 1M one-pass runner), `benches/receipts/` (receipt + raw logs incl. after-fix runs), `.github/workflows/ci.yml` (`bench-smoke`) | PARTIAL (F1/F2/F4 fixed with same-machine before/after receipts: FS put 958→266 910 ops/s, SQLite 1M scan 847/s→2.21M/s, SQLite 1M open 310 ms→39 µs; F3 MISS ×1.3 justified with decomposition, F5 marginal at fsync floor; blocking bench-regression workflow + comparator + interim baseline shipped, enforcement pending runner provisioning → follow-up) |
| R12.2 | Memory: no range materialization, connection/txn overhead probes, lifecycle RSS gates | `crates/boa_idb/tests/memory_gates_tests.rs` (allocation-counter gates in CI: ≤64 KiB marginal conn, ≤8 KiB txn, store 1M/1M/200k Next+Prev + index Next/Prev peak < 1 MiB all backends (dhat gates, ADR-014; `BOA_IDB_FULL_MATRIX=1` raises FS/index to 1M with receipts), subquadratic `continue()`, lifecycle), nightly `cursor-matrix-1m` job + massif RSS gate, `boa-idb-cli` byte counters, README memory model | PASS on local evidence (PR gates green incl. full 1M matrix 12/12 with local receipts docs/reviews/RECEIPT-MATRIX-1M-20260906.md; nightly cursor-matrix-1m / massif / long-lifecycle run URLs pending inaugural scheduled runs) |
...
| R13.1 | Test levels: perf, memory, fuzz, crash | `criterion` + `scan-1m` (perf), `memory_gates_tests` + massif (memory), 5 libFuzzer targets × 48 min nightly (fuzz), kill-worker 200 nightly (crash) | PARTIAL (all levels wired and green locally incl. fuzz-target compile; 4 h Linux fuzz evidence pending inaugural nightly run) |
| R13.2 | Coverage thresholds (`boa_idb_core` ≥90 %, `boa_idb` ≥80 %, `llvm-cov` gate) | Nightly `llvm-cov` (tests + 3 WPT runs) with LCOV artifact + absolute threshold gate (core 90 / boa_idb 80, schema bug fixed); measured actuals core 90.1 / boa_idb 80.9 (docs/reviews/coverage-20260907/, clean accumulation) | PARTIAL (code at 90.1/80.9 with local evidence + enforcing absolute gate; CI coverage artifact with command/SHA/percentages pending inaugural nightly run) |
| R13.1-fuzz | Fuzz targets SCF/key/WAL/stateful (part of the fuzz level above) | `fuzz/fuzz_targets/` (5 targets incl. `fuzz_wal_recovery`, `fuzz_stateful_txn` with model oracle), nightly 4 h | PARTIAL (targets compile via stable `cargo check`; first 4 h evidence pending inaugural nightly run; local `cargo-fuzz` link needs the Linux ASan runtime) |
| R13.4.1 | Backend differential behavior (10 000 nightly scenarios) | `differential_model_tests.rs` (10 000 proptest cases), `differential_fs_tests.rs`, backend suites, nightly full-suite run | PASS (10k suites green; nightly re-runs with long-lifecycle env) |
| R13.6.1–R13.6.2 | GC/resource lifecycle: RSS growth, handle release | `memory_gates_tests` lifecycle (50 CI / 10 000 nightly) + JS state-release smoke, massif peak gate nightly, WPT isolated-context runner, backend cleanup tests | PASS |
...
* R12–R13 (M7-B): same ids, statuses updated above (R12.1 PARTIAL: gate wired, enforcement + labelled evidence pending; R12.2 PASS on local 1M receipts, nightly run URL pending; R13.1/R13.1-fuzz PARTIAL: 4 h Linux fuzz pending; R13.2 PARTIAL: local 90.1/80.9 + enforcing gate, CI artifact pending; R12.3/R13.4.1/R13.6 PASS).
