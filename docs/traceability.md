...
| R13.2 | Coverage thresholds (`boa_idb_core` ≥90 %, `boa_idb` ≥80 %, `llvm-cov` gate) | Nightly `llvm-cov` (tests + 3 WPT runs) with LCOV artifact + per-package regression ratchet (core 78 / boa_idb 73); measured actuals core 80.5 / boa_idb 75.5 | PARTIAL (ratchet blocks regression; absolutes need the coverage-drive debt: dead `engine::{connection,cursor,registry,transaction,request}` modules + API edge branches; no broad ignores used) |
