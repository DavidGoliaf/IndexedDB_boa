...
| R13.1 | Test levels: perf, memory, fuzz, crash | `criterion` + `scan-1m` (perf), `memory_gates_tests` + massif (memory), 5 libFuzzer targets × 48 min nightly (fuzz), kill-worker 200 nightly (crash) | PASS (CI run 34240365417 on main @ 72757a8, contains baseline 72827e8; M7-handoff §5) |
| R13.2 | Coverage thresholds (`boa_idb_core` ≥90 %, `boa_idb` ≥80 %, `llvm-cov` gate) | Nightly `llvm-cov` (tests + 3 WPT runs) with LCOV artifact + absolute threshold gate (core 90 / boa_idb 80, schema bug fixed); measured actuals core 90.1 / boa_idb 80.9 (docs/reviews/coverage-20260907/, clean accumulation) | PASS (CI run 34240365417 on main @ 72757a8, contains baseline 72827e8; M7-handoff §5) |
| R13.1-fuzz | Fuzz targets SCF/key/WAL/stateful (part of the fuzz level above) | `fuzz/fuzz_targets/` (5 targets incl. `fuzz_wal_recovery`, `fuzz_stateful_txn` with model oracle), nightly 4 h | PASS (CI run 34240365417 on main @ 72757a8, contains baseline 72827e8; M7-handoff §5) |
...
* R12–R13 (M7-C): same ids, statuses updated above (R12.1 PASS on PR #1/run 34206701559, see M7-handoff §4; R12.2/R13.1/R13.1-fuzz/R13.2 PASS on CI run 34240365417 (baseline-containing SHA), see M7-handoff §5; R12.3/R13.4.1/R13.6 PASS).
