...
| R13.2 | Coverage thresholds (`boa_idb_core` ≥90 %, `boa_idb` ≥80 %, `llvm-cov` gate) | Nightly `llvm-cov` (tests + 3 WPT runs) with LCOV artifact + absolute threshold gate (core 90 / boa_idb 80, schema bug fixed); measured actuals core 90.37 / boa_idb 80.52 | PARTIAL (code at 90.37/80.52 with local evidence + enforcing absolute gate; CI coverage artifact with command/SHA/percentages pending inaugural nightly run) |
