...
| R12.1 | Benchmark harness, 7 §12.1 scenarios (SQLite/FS + memory control), receipts, CI smoke, M7-B fixes | `crates/boa_idb/benches/backends.rs` (scan ids carry `N`, diagnostic), `examples/scan-1m.rs` (normative 1M one-pass runner), `benches/receipts/` (receipt + raw logs incl. after-fix runs), `.github/workflows/ci.yml` (`bench-smoke`) | PASS (CI: F1/F2/F4 fixed with same-machine before/after receipts: FS put 958→266 910 ops/s, SQLite 1M scan 847/s→2.21M/s, SQLite 1M open 310 ms→39 µs; F3 MISS ×1.3 justified with decomposition, F5 marginal at fsync floor; labelled runner boa-bench-01 + baseline 72827e8 + required bench-regression PR check green on PR #1/run 34206701559, M7-handoff §4) |
...
* R12–R13 (M7-C): same ids, statuses updated above (R12.1 PASS on PR #1/run 34206701559, see M7-handoff §4; R12.2/R13.1/R13.1-fuzz/R13.2 PASS on CI run 34092345578, see M7-handoff §5; R12.3/R13.4.1/R13.6 PASS).
